...
//  configDisplay.set = (OMX_DISPLAYSETTYPE)(OMX_DISPLAY_SET_TRANSFORM | OMX_DISPLAY_SET_LAYER );
  configDisplay.set = (OMX_DISPLAYSETTYPE)(OMX_DISPLAY_SET_TRANSFORM | OMX_DISPLAY_SET_LAYER | OMX_DISPLAY_SET_ALPHA );
  configDisplay.layer = m_layer;
  configDisplay.transform = m_transform;
  configDisplay.alpha = 0x00000040;
...
bool COMXVideo::SetAlpha( int alpha )
{
  OMX_ERRORTYPE omx_err   = OMX_ErrorNone;
  OMX_CONFIG_DISPLAYREGIONTYPE configDisplay;
  OMX_INIT_STRUCTURE(configDisplay);
  ...
  configDisplay.alpha = alpha & 0xFF;
  omx_err = m_omx_render.SetConfig(OMX_IndexConfigDisplayRegion, &configDisplay);
  if(omx_err != OMX_ErrorNone)
  {
    CLog::Log(LOGWARNING, "%s::%s - could not set alpha : %d", CLASSNAME, __func__, m_transform);
    return false;
  }
  return true;
}
