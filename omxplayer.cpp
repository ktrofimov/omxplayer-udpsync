...
bool              m_is_fadein_sent      = false;
bool              m_is_fadeout_sent     = false;
int64_t           m_duration            = 0;
int64_t           frame                 = 0;
int64_t           m_fade_sound          = 0;
...
//  printf("         -a / --alang language          audio language        : e.g. ger\n");
  ...
  printf("              --fi n                    fade in (default 0 seconds)\n");
  printf("              --fo n                    fade out (default 0 seconds)\n");
  ...
  printf("              --pause                   Pause at start waiting for play command\n");
}
...
void fadeSound( void )
{
  m_fade_sound = frame + m_fadeout*DVD_TIME_BASE;
  printf( "Fading sound mfs: %lld fr: %lld fo: %d\n", m_fade_sound, frame, m_fadeout );
}
...
void SetSpeed(int iSpeed)
{
    if(!m_av_clock)
    {
	printf("Can't set speed - no av_clock!\n");
	return;
    }
  m_av_clock->OMXPause();
  m_omx_reader.SetSpeed(0);
  ...
  printf("Set speed: %d, current speed: %d \t", iSpeed, m_av_clock->OMXPlaySpeed() );
}

void JumpToPos( double Position )
{
    double oldPos, newPos;
    newPos = Position * 1e-6;
    oldPos = m_av_clock->OMXMediaTime()*1e-6;
    m_incr = newPos - oldPos;
printf( "OLD POS:%7.2f NEW POS:%7.2f INCR:%7.2f\n", oldPos, newPos, m_incr );
}
...
int main(int argc, char *argv[])
{
  ...
  const int sync_server_opt = 0x20d;
  const int sync_port_opt   = 0x20e;
  const int sync_client_opt = 0x20f;
  const int server_address_opt = 0x210;
  const int sync_num_clients_opt = 0x211;
  const int sync_verbose_opt = 0x212;
  const int fi_opt           = 0x213;
  const int fo_opt           = 0x214;
  const int pause_opt        = 0x215;
  ...
  struct option longopts[] = {
    ...
    { "fi",           required_argument,  NULL,          fi_opt },
    { "fo",           required_argument,  NULL,          fo_opt },
    ...
    { "pause",         no_argument,        NULL,         pause_opt },
    { 0, 0, 0, 0 }
  };
 ...
 */
  ...
  OMXPlayerSync *omxPlayerSync=NULL;
  ...
  m_threshold = atof(optarg);
      ...
      case sync_verbose_opt:
        ...
        break;
      case fi_opt:
        m_fadein = std::min(atoi(optarg), 20);
        break;
      case fo_opt:
        m_fadeout = std::min(atoi(optarg), 20);
        break;
      case pause_opt:
        m_pause = true;
        break;
  ...
  m_duration      = m_omx_reader.GetDuration();
  ...
  if( m_fadein == 0 )
    m_player_video.SetAlpha( ALPHA_OPAQUE );
  else
    m_player_video.SetAlpha( ALPHA_TRANSPARENT );
  ...
  while(!m_stop)
  {
    ...
    // Fade in & fade out
    static bool fade_set=0;
    frame = (int64_t)( m_av_clock->OMXMediaTime() );

    // printf( "%d: Pos: %3.2f\n", tcp_port, ((double)frame)/DVD_TIME_BASE );

    if( m_fadein>0 && frame>=0 && frame <= m_fadein*DVD_TIME_BASE )
    {
	if( m_layer > 1 )
	{
	    alpha = (int)( frame*ALPHA_OPAQUE/(m_fadein*DVD_TIME_BASE) ) & 0xFF;
	...
	else
	{
        ...
        m_Volume = ( frame*3000/(m_fadein*DVD_TIME_BASE) )-3000;
        m_player_audio.SetVolume(pow(10, m_Volume / 2000.0));
    }
    if( frame > m_fadein*DVD_TIME_BASE && fade_set==0 )
    {
	if( !m_is_fadein_sent && (m_is_sync_server||m_is_sync_client) )
	    omxPlayerSync->atFadingIn();
	m_is_fadein_sent = true;
	fade_set=1;
	alpha=ALPHA_OPAQUE;
	m_player_video.SetAlpha( alpha );
	m_Volume = 0;
    ...
    if( m_fadeout>0 && frame >= m_duration-m_fadeout*DVD_TIME_BASE )
    {
	if( !m_is_fadeout_sent && m_is_sync_server )
	    omxPlayerSync->atFadingOut();
	m_is_fadeout_sent = true;
	if( m_layer > 1 )
	{
	    alpha=(int)( (m_duration-frame)*ALPHA_OPAQUE/(m_fadeout*DVD_TIME_BASE) ) & 0xFF;
	    m_player_video.SetAlpha( alpha );
	}
	else
	    m_player_video.SetAlpha( ALPHA_OPAQUE );
        m_Volume = ( (m_duration-frame)*3000/(m_fadeout*DVD_TIME_BASE) )-3000;
        m_player_audio.SetVolume(pow(10, m_Volume / 2000.0));
    }
    // Sound fading
    if( m_fade_sound>0 ) // && frame >= m_duration-m_fade_sound-m_fadeout*DVD_TIME_BASE )
    {
//  printf( "mfs: %lld f: %lld\n", m_fade_sound, frame );
        m_Volume = ( (m_fade_sound-frame)*3000/(m_fadeout*DVD_TIME_BASE) )-3000;
        m_player_audio.SetVolume(pow(10, m_Volume / 2000.0));
	// printf( "Vol: %d\n", m_Volume );
        if( frame >= m_duration-m_fade_sound )
        {
            m_fade_sound = 0;
            printf( "Stop fading sound\n" );
        }
    }
    ...
    {
       omxPlayerSync -> update ( alpha );
	// test code
	//printf( "F:%d, FI:%d, FO:%d, L:%d, A:%d\n", (int)(frame/DVD_TIME_BASE), (int)(m_fadein), (int)(m_fadeout),  m_layer, alpha );
    }
    ...
    {
      case KeyConfig::ACTION_SHOW:
	if( alpha < 0xFF )
	    m_player_video.SetAlpha( ++alpha );
        break;
      case KeyConfig::ACTION_BLUR:
	if( alpha > 0 )
	    m_player_video.SetAlpha( --alpha );
        break;
...
//  if (m_stop)
  ...
  printf( "Stop av clock..." );
  m_av_clock->OMXStop();
  m_av_clock->OMXStateIdle();
  printf( " Done\n" );

  printf( "Close decoders... subtitles... " );
